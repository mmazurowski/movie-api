...
import { asyncLocalStorage, requestIdMiddleware } from '@application/server/rest/middlewares/request-id.middleware';
...
export class BindSystemCoreDependenciesProcedure extends Procedure<AwilixContainer, AwilixContainer> {
   async run(container: AwilixContainer): Promise<AwilixContainer> {
       container.register({
           ...
           fileSystemMoviesPath: Awilix.asValue(join(__dirname, '..', '..', '..', 'database', 'db.json')),
       });

       const awilixPatterns = [process.env.NODE_ENV !== 'production' ? 'src/**/**/**/**/**/*.action.ts' : 'dist/**/**/**/**/**/*.action.js'];
