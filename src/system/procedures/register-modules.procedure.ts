...
import { Command, CommandHandler, CQRSBus, Query, QueryHandler } from '@lunar-flight-v/command-module';
...
export class RegisterModulesProcedure extends Procedure<AwilixContainer, AwilixContainer> {
    async run(container: AwilixContainer): Promise<AwilixContainer> {
        ...
        container.register('controllers', registerAsArray(applicationControllers.map((controller) => Awilix.asClass(controller).singleton())));
        ...
        container.register('commandHandlers', registerAsArray(applicationCommandHandlers.map((handler) => Awilix.asClass(handler))));
        container.register('queryHandlers', registerAsArray(applicationQueryHandlers.map((handler) => Awilix.asClass(handler))));
        container.resolve<CommandHandler<Command<unknown>>[]>('commandHandlers').forEach((handler) => cqrsbus.registerHandler(handler));
        container.resolve<QueryHandler<Query<unknown>>[]>('queryHandlers').forEach((handler) => cqrsbus.registerHandler(handler));
