...
export const errorMiddleware = ({ logger, asyncLocalStorage }: Dependencies): ErrorRequestHandler => (err, req, res, _) => {
    const isCelebrate = isCelebrateError(err);

    logger.error(`[Request TransactionID: ${asyncLocalStorage.getStore().get(TRANSACTION_ID_HTTP_HEADER)}] [HTTP Error ${isCelebrate ? 422 : err.code || 500} at ${req.method} ${process.env.APP_HOST}:${process.env.APP_PORT}${req.path}] ${err.toString()}`);
